...
docker run --name wenet_task_manager_dev -v /var/run/docker.sock:/var/run/docker.sock -v ${HOME}/.m2/repository:/root/.m2/repository  -v ${PWD}:/app -it internetofus/task-manager:dev /bin/bash
popd >/dev/null