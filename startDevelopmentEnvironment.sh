...
if [ -f /.dockerenv ]; then
   echo "You can not start the development environment inside a docker container"
	...
	docker run --rm --name wenet_task_manager_dev -v /var/run/docker.sock:/var/run/docker.sock -v "${HOME}/.m2/repository":/root/.m2/repository  -v "${PWD}":/app -p 5002:5005 -it internetofus/task-manager:dev /bin/bash
